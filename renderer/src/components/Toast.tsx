    ...
    <div className="fixed bottom-10 left-5 right-5 z-[200] flex flex-col gap-2 items-end pointer-events-none">
      {toasts.map((t) => {
        ...
        return (
          <div
            key={t.id}
            className="dm-overlay pointer-events-auto panel-surface bg-[var(--panel-bg)] border rounded-xl shadow-2xl px-3.5 py-2.5 flex items-center gap-3 min-w-[240px] max-w-[min(380px,100%)] fade-in-up select-none"
            style={{ borderColor: `color-mix(in srgb, ${tone.color} 40%, var(--border))` }}
          >
            <Icon className="w-4 h-4 shrink-0" style={{ color: tone.color }} />
            <span
              className="text-[calc(12px*var(--font-scale))] text-[var(--text-primary)] flex-1 min-w-0 leading-snug [overflow-wrap:anywhere]"
              title={t.message}
            >
              {t.message}
            </span>
