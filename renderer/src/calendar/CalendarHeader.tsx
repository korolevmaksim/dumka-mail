...
export function CalendarHeader(props: CalendarHeaderProps) {
  return (
    <header className="dm-calendar-header dm-toolbar flex min-h-[58px] shrink-0 items-center gap-3 border-b border-[var(--border)] bg-[var(--panel-bg)] px-3">
      ...
      <div className="dm-control flex items-center rounded-md border border-[var(--border)] bg-[var(--app-bg)]">
      ...
      <div className="dm-control ml-auto hidden items-center rounded-lg border border-[var(--border)] bg-[var(--app-bg)] p-0.5 lg:flex" aria-label="Calendar view">
        {CALENDAR_VIEW_OPTIONS.map(option => (
          <button key={option.id} type="button" aria-pressed={props.view === option.id} onClick={() => props.onChangeView(option.id)} title={`${option.label} (${option.shortcut})`} className={`dm-calendar-view-button rounded-md px-2.5 py-1.5 text-[calc(10px*var(--font-scale))] font-medium ${props.view === option.id ? 'bg-[var(--raised-surface)] text-[var(--text-primary)] shadow-sm' : 'text-[var(--text-tertiary)] hover:text-[var(--text-primary)]'}`}>{option.label}</button>
        ))}
      </div>
