...
export function CalendarHeader(props: CalendarHeaderProps) {
  ...
  return (
    <header className="flex min-h-[64px] shrink-0 items-center gap-3 border-b border-[var(--border)] bg-[var(--panel-bg)] px-3" style={dragStyle}>
      ...
      <div style={noDragStyle} className="flex items-center rounded-lg border border-[var(--border)] bg-[var(--app-bg)]">
      ...
      <div style={noDragStyle} className="ml-auto hidden items-center rounded-lg bg-[var(--raised-surface)] p-1 shadow-[inset_0_0_0_1px_var(--border)] lg:flex" role="group" aria-label="Calendar view">
        {CALENDAR_VIEW_OPTIONS.map(option => (
          <button key={option.id} type="button" aria-pressed={props.view === option.id} onClick={() => props.onChangeView(option.id)} title={`${option.label} (${option.shortcut})`} className={`cursor-pointer rounded-md px-2.5 py-1.5 text-[calc(10px*var(--font-scale))] font-medium transition-[background-color,color,box-shadow] duration-150 focus-visible:outline-2 focus-visible:outline-[var(--accent)] ${props.view === option.id ? 'bg-[var(--panel-bg)] text-[var(--text-primary)] shadow-[0_1px_4px_rgba(0,0,0,0.12)]' : 'text-[var(--text-tertiary)] hover:bg-[var(--hover-row)] hover:text-[var(--text-primary)]'}`}>{option.label}</button>
        ))}
      </div>
