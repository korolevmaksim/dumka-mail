...
function accessibleEventLabel(event: CalendarEvent, calendar?: CalendarListEntry): string {
  ...
  return [event.summary, timing, calendar?.summary, event.location, event.status === 'pending' ? 'waiting to sync' : null]
    .filter(Boolean)
    .join(', ');
}
...
export function CalendarEventChip({ event, calendar, compact = false, draggable = true, onSelect }: CalendarEventChipProps) {
  ...
  return (
    ...
    >
      ...
      {event.conferenceUrl && <span className="ml-auto shrink-0 opacity-80" aria-label="Video meeting">⌁</span>}
      {event.status === 'pending' && <span className="ml-auto shrink-0 text-[8px] uppercase opacity-75">sync</span>}
    </button>
  );
}
