...
- Agentic mail layer with a durable Today operator workspace, locally persisted daily briefings and Agent Review Queue state, Follow-up Radar for sent mail awaiting replies, source-cited AI-assisted triage, opt-in proactive draft previews, notification filtering and actions, rule dry-runs, one-click unsubscribe support, local security snapshots, tracker stripping, and phishing-link warnings.
